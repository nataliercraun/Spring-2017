...
private:
    ...
    // Master map of variable ID to variable value 
    // Changes as the user changes each variable 
    std::map<int,int> varValues_; 

    // Additional map of each variable to ONLY the clauses that contain that variable 
    // key: var ID value: set of all of the clauses the variable is in
    std::map<int, std::set<Clause*> > varToClauses_; 

    // Vector of all of the given clauses 
    std::vector<Clause*> clauses_;

    // Tracks how many clauses are satisfied by the current variable assignment 
