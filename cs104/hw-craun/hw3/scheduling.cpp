...
void scheduleJobs(int *resources,
    ...
    for (int i = 0; i < numResources; i++) {
        ...
        if (!jobqueue.empty() && resources[i] >= jobqueue.front()) {  
            scheduleAndPrintJob(iteration, jobqueue.front(), i); 
            resources[i] = resources[i] - jobqueue.front(); 
            jobqueue.pop();
...
int main(int argc, char *argv[]) {
    ...
    while( (!inf.fail()) || (!jobqueue.empty()) ) {
        ...
        if (jobqueue.empty()) {
            ...
            QueueInt newqueue = readLine(inf); 
            jobqueue.concatenate(newqueue);
        }
