...
bool readGrid(istream& instream, vector<vector<char> >& grid )
{
  ...
  while (getline (instream, line)) {
    ...
    while (ss >> temp) {
      ...
      oneline = true; // set true if at least 1 row of more than 0 characters is read 
    }
  ...
  for (unsigned int i = 0; i < grid.size(); i++) {
    ...
    if (grid[i].size() != grid[0]. size()) {
      oneline = false; // set false if rows do not contain the same # of characters 
    }
  }
		 ...
		 unsigned int currWordIndex)
{
  ...
  //recursive case 
      ...
      // check if the next location is within the grid and contains the correct letter 
       
      if (currLoc.row > 0 && currLoc.col > 0 
          && currLoc.row < (int)grid.size() && currLoc.col < (int)grid[0].size() 
          && word[currWordIndex] == grid[currLoc.row][currLoc.col]) {  

        // move to the next location indicated by delta 

        currLoc.row = currLoc.row + delta.row; 
        currLoc.col = currLoc.col + delta.col; 

        // recurse to check if the next letter is in the same direction as the last 
